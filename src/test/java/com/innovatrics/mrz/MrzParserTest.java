...
public class MrzParserTest {
    ...
    public void testComputeCheckDigit() {
        ...
        assertEquals(
                2,
                MrzParser.computeCheckDigit("D231458907<<<<<<<<<<<<<<<34071279507122<<<<<<<<<<"));
    ...
    public void testValidCheckDigit() {
        ...
        String CzechPassport =
                "P<CZESPECIMEN<<VZOR<<<<<<<<<<<<<<<<<<<<<<<<<\n"
                        + "99003853<1CZE1101018M1207046110101111<<<<<94";
        assertTrue(MrzParser.parse(CzechPassport).validDateOfBirth);
        assertTrue(MrzParser.parse(CzechPassport).validExpirationDate);
        assertTrue(MrzParser.parse(CzechPassport).validDocumentNumber);
        assertTrue(MrzParser.parse(CzechPassport).validComposite);

        String GermanPassport =
                "P<D<<MUSTERMANN<<ERIKA<<<<<<<<<<<<<<<<<<<<<<\n"
                        + "C01X01R741D<<6408125F2010315<<<<<<<<<<<<<<<9";
        assertTrue(MrzParser.parse(GermanPassport).validDateOfBirth);
        assertTrue(MrzParser.parse(GermanPassport).validExpirationDate);
        assertTrue(MrzParser.parse(GermanPassport).validDocumentNumber);
        assertFalse(
                MrzParser.parse(GermanPassport)
                        .validComposite); // yes, this specimen has intentationally wrong check
        // digit
    }
    ...
    public void testNameToMrz() {
        ...
        assertEquals(
                ...
                MrzParser.nameToMrz("Papandropoulous", "Jonathoon Alec", 31));
        assertEquals(
                "NILAVADHANANANDA<<CHAYAPA<DEJ<K",
                MrzParser.nameToMrz("Nilavadhanananda", "Chayapa Dejthamrong Krasuang", 31));
        assertEquals(
                "NILAVADHANANANDA<<ARNPOL<PETC<C",
                MrzParser.nameToMrz("NILAVADHANANANDA", "ARNPOL PETCH CHARONGUANG", 31));
        assertEquals(
                "BENNELONG<WOOLOOMOOLOO<W<W<<D<P",
                MrzParser.nameToMrz(
                        "BENNELONG WOOLOOMOOLOO WARRANDYTE WARNAMBOOL", "DINGO POTOROO", 31));
    }

    @Test
    public void testValidDates() {
        String validBirthDateMrz =
                "P<GBRUK<SPECIMEN<<ANGELA<ZOE<<<<<<<<<<<<<<<<\n"
                        + "9250764733GBR8809117F2007162<<<<<<<<<<<<<<08";
        MrzRecord record = MrzParser.parse(validBirthDateMrz);
        assertTrue(record.dateOfBirth.isDateValid());
        assertTrue(record.expirationDate.isDateValid());
        assertTrue(record.validDateOfBirth);
        assertTrue(record.validExpirationDate);
    }

    @Test
    public void testMrzInvalidBirthDate() {
        String invalidBirthDateMrz =
                "P<GBRUK<SPECIMEN<<ANGELA<ZOE<<<<<<<<<<<<<<<<\n"
                        + "9250764733GBR8809417F2007162<<<<<<<<<<<<<<08";
        MrzRecord record = MrzParser.parse(invalidBirthDateMrz);
        assertFalse(record.dateOfBirth.isDateValid());
        assertFalse(record.validDateOfBirth);
    }

    @Test
    public void testMrzInvalidExpiryDate() {
        String invalidExpiryDateMrz =
                "P<GBRUK<SPECIMEN<<ANGELA<ZOE<<<<<<<<<<<<<<<<\n"
                        + "9250764733GBR8809117F2007462<<<<<<<<<<<<<<08";
        MrzRecord record = MrzParser.parse(invalidExpiryDateMrz);
        assertFalse(record.expirationDate.isDateValid());
        assertFalse(record.validExpirationDate);
    }

    @Test
    public void testUnparseableDates() {
        String unparseableDatesMrz =
                "P<GBRUK<SPECIMEN<<ANGELA<ZOE<<<<<<<<<<<<<<<<\n"
